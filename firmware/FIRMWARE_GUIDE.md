...
### 1.1 Core Assignment
- **Core 0 (`fetchTask`):** background HTTP/HTTPS to external APIs (Finnhub stocks; Open-Meteo geocoding + weather). Pinned to Core 0 with an 8 KB stack (`FETCH_TASK_STACK = 8192`).
...
### 6.4 Geocoding
`fetchWeatherImpl()` resolves each location string to lat/lon via Open-Meteo's geocoding API on first use, caching in `resolved[]` (RAM). `applyPendingLocations()` invalidates the cache (`resolved[i].ok = false`). A trailing `", XX"` is used as an `admin1` / `country_code` filter to disambiguate duplicate city names.
