...
BLE writes set the display mode. When the user requests stocks or weather without the prerequisite config (WiFi creds, Finnhub key), the device enters a transient `MODE_SETUP` that displays configuration hints — see "Setup mode" below.
...
- **BLE (`LED-Ticker-XXXX`, where `XXXX` is the low 2 bytes of the chip MAC)** — primary control plane for WiFi creds, Finnhub key, tickers, messages, locations, mode, and commands. On first boot with no WiFi configured, the device enters `MODE_SETUP` (see Architecture Notes).
...
- **Boot sequence matters** (`setup()`): `initDisplay()` → load user config from NVS (`wifi`, `apikey`, `msgs`, `tickers`, `locs`) → `buildDeviceName()` → enter `MODE_SETUP(SETUP_BOOT)` if WiFi is unconfigured, else `enterAllMode()` → `showNext()` so the display is live before networking → `connectWifi()` → `initTime()` → `initBLE()` → `triggerFetch()`.
- **Setup mode** (`MODE_SETUP`): Transient mode shown when prereqs are missing. Entered on boot if WiFi is unconfigured (`SETUP_BOOT`), or when the user requests `MODE_STOCKS`/`MODE_WEATHER` without the needed config (`SETUP_STOCKS`/`SETUP_WEATHER`), or after `cmd=reset` wipes config. Cycles two scroll frames (contextual hint + BLE device name like `LED-Ticker-AB12`). Exits when prereqs are satisfied (resumes the requested mode — `applyPendingWifi`/`applyPendingApiKey` call `exitSetupIfReady()`), when the user sends an explicit `mode=` write, or after `SETUP_TIMEOUT_MS` (60s) of no BLE activity (falls to `MODE_ALL`). Every BLE characteristic `onWrite` resets the activity timer.
...
- **Reset semantics**: `cmd=reset` clears *all* NVS namespaces (including `wifi` and `apikey`), zeroes the in-memory `nvsWifiSsid`/`nvsWifiPass`/`nvsApiKey` buffers (otherwise `wifiConfigured()` and `apiKeyConfigured()` keep returning true until reboot), calls `WiFi.disconnect()` to drop the live session, re-seeds tickers and locations from `config.h`, and enters `MODE_SETUP(SETUP_BOOT)`. After reset the device needs WiFi and API key reconfigured over BLE.
- **Display gating** (`showNext()`): in `MODE_ALL`, stocks show only when WiFi+API key are configured *and* `stockCount > 0`; weather shows only when WiFi is configured *and* `weatherCount > 0`; otherwise the cycle skips to messages (which always has fallback text from `config.h`). Explicit `MODE_STOCKS`/`MODE_WEATHER` requests with missing prereqs are diverted to `MODE_SETUP` by `applyPendingMode`, so `showNext()` for those modes assumes prereqs are met and only handles the "loading" case.
...
- **Main loop** is cooperative: apply pending BLE updates → update status LED → check setup-mode timeout → advance display animation → fetch every `FETCH_INTERVAL_MS` (5 min) if WiFi is up. No `delay()` inside `loop()`.
...
Advertised as `LED-Ticker-XXXX` (low 2 bytes of chip MAC), service UUID `4fafc201-1fb5-459e-8fcc-c5c9c331914b`. All characteristics except `Command` are read/write — reads return current state (e.g. `Tickers` returns the comma-joined ticker list, `WiFi` returns SSID only); `Command` is write-only. Payload formats and UUIDs are documented in `README.md`. WiFi payload is `SSID|password` split on the first `|` (passwords may contain `|`; SSIDs may not).
